...
define
	...
	% muffle(start:〈duration〉 finish:〈duration〉 intensity:〈factor〉 〈music〉):

	fun {Muffling Samples Index StartI FinishI Intensity}
		case Samples
		of nil then nil
		[] H|T then
			if (Index >= StartI) then
				if (Index < FinishI) then
					H*Intensity | {Muffling T Index+1 StartI FinishI Intensity}
				else
					H | {Muffling T Index+1 StartI FinishI Intensity}
				end
			else
				H | {Muffling T Index+1 StartI FinishI Intensity}
			end
	...
	fun {Muffle Start Finish Intensity Music P2T}
		StartI = {Float.toInt Start*44100.0}
		FinishI = {Float.toInt Finish*44100.0}
		Samples = {Mix P2T Music}
	in
		{Muffling Samples 0 StartI FinishI Intensity}
	end
	...
	in
		...
		[] muffle(start:S finish:F intensity:I Music)|T then
			if(Args.'extension' == true) then
				{Append {Muffle S F I Music P2T} {Mix P2T T}}
			else
				nil
			end
		else
