...
define
   ...
   fun {NoteToExtended Note}
	...
	in
		...
		{AssertEquals {Mix P2T MusicLoop} E 'TestRepeat: below size'}
		{AssertEquals {Mix P2T MusicLoop2} E2 'TestRepeat: above size'}
	end

	proc {TestClip P2T Mix}
		Samples = [0.1 ~0.2 ~0.7 0.2 0.4 0.9 ~0.9 0.3]
		Music = [samples(Samples)]
		MusicClip = [clip(low: ~0.3 high:0.6 Samples)]
		E = [0.1 ~0.2 ~0.3 0.2 0.4 0.6 ~0.3 0.3]
		MusicClip2 = [clip(low: 0.3 high:0.6 Samples)]
		E2 = [0.1 ~0.2 ~0.3 0.2 0.4 0.6 ~0.3 0.3 0.6]
		MusicClip3 = [clip(low: 0.5 high:0.0 Samples)]
		E3 = nil
	in
	...
	in
		{System.show '----------- TestCut -------------'}
		{AssertEquals {Mix P2T MusicCut} E 'TestClip: basic'}
	end
